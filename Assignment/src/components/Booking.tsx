...
const Booking = () => {
  ...
  const onSubmit: SubmitHandler<IBooking> = async (booking) => {
    ...
    if (+payment == 0) {
      ...
      const infoMail = {
        to_name: infoBooking.name,
        from_name: "Nguyễn Huy Tới",
        message: `Cảm ơn bạn bạn đã tin tưởng đặt vé. Mã vé của bạn là: ${data._id}`,
        to_mail: infoBooking.email,
      };
      emailjs
        .send("service_0rid2hf", "template_5rhhgrn", infoMail, {
          publicKey: "5wF194SnyVv2P39lY",
        })
        .then((respon) => {
